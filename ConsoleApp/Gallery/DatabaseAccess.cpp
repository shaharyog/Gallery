...
void DatabaseAccess::createAlbum(const Album& album)
{
	if (doesAlbumExists(album.getName()))
		throw MyException("Album '" + album.getName() + "' already exists!");
...
Album DatabaseAccess::openAlbum(const std::string& albumName)
{
	...
	for (Picture& picture : pictures) {
		std::set<int> user_tags = getPictureTags(album.getName(), picture);

		for (const int user_id : user_tags)
...
void DatabaseAccess::addPictureToAlbumByName(const std::string& albumName, const Picture& picture)
{
	...
	if (doesPictureExistsInAlbum(albumName, picture.getName()))
		throw MyException("Picture " + picture.getName() + " already exists in album " + albumName);

	const std::string addPictureToAlbumSQL =
		"INSERT INTO PICTURES (ID, NAME, LOCATION, CREATION_DATE, ALBUM_ID) "
		"VALUES (" + std::to_string(picture.getId()) +
...
void DatabaseAccess::tagUserInPicture(const std::string& albumName, const std::string& pictureName, int userId)
{
	...
	if (doesUserTaggedPicture(userId, pictureID))
		throw MyException("User " + std::to_string(userId) + " is already tagged in picture " + pictureName);
...
void DatabaseAccess::untagUserInPicture(const std::string& albumName, const std::string& pictureName, int userId)
{
	...
	if (!doesUserTaggedPicture(userId, pictureID))
		throw MyException("User " + std::to_string(userId) + " is not tagged in picture " + pictureName);
...
{
	if (doesUserExists(user.getId()))
		throw MyException("User '" + user.getName() + "' with id '" + std::to_string(user.getId()) + "' already exists!");

	const std::string query = "INSERT INTO USERS (ID, NAME) VALUES (" + std::to_string(user.getId()) + ", '" + user.getName() + "' );";
	runSQL(query);
}
...
std::list<Picture> DatabaseAccess::getTaggedPicturesOfUser(const User& user)
{
	...
	for (Picture& picture : pictures) {
		std::set<int> user_tags = getPictureTags(picture);
...
Album DatabaseAccess::getAlbum(const std::string& albumName, std::optional<int> userId)
{
	Album album(-1, "");
...
std::list<Picture> DatabaseAccess::getAlbumPictures(const Album& album)
{
...
int DatabaseAccess::getPictureID(const std::string& albumName, const std::string& pictureName) const
{
	...
	if (pictureID == -1)
		throw ItemNotFoundException("Picture does not exist: ", pictureName);

	return pictureID;
}

std::set<int> DatabaseAccess::getPictureTags(const std::string& albumName, const Picture& picture) const
{
	if (!doesPictureExistsInAlbum(albumName, picture.getName()))
...
std::set<int> DatabaseAccess::getPictureTags(const Picture& picture) const
{
